...
out vec2 fTexCoord; // Fragment Shader로 보낼 텍스처 좌표 데이터
...
void main() {
    gl_Position = projection * view * rotation * model * vec4(vPos, 1.0);
	fPos = vec3(rotation * model * vec4(vPos, 1.0));
    fNormal = transpose(inverse(mat3(rotation * model))) * vNormal;
    fColor = vColor;
    fTexCoord = vTexCoord;
}